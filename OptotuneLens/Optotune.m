classdef Optotune < handle
    ...
    methods
        ...
        function setCurrent(lens, ci) %% Set current in mA via ci variable
            %% Set current %%%%%%%%
            set_i = abs(floor(ci*(lens.max_bin+1) / lens.max_current));
%             set_i_bin=dec2bin(set_i,16);
            LB = mod(set_i,256); %% low byte
            HB = (set_i-LB)/256; %% high byte
            if ci<0
                set_i_bin=dec2bin(2^16-set_i,16);
                high_bin = set_i_bin(1:8);
                low_bin = set_i_bin(9:end);           
                HB = bin2dec(high_bin);
                LB = bin2dec(low_bin);
            end
